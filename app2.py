...
#     description="API for generating resumes and analyzing them against job descriptions",
#     version="1.0.0"
# )
# rate_limit_records = defaultdict(float)

# # ✅ This will act like our "middleware" but as a dependency
...
#     print(f"Request from {client_ip} allowed at {current_time}")
# # Add CORS middleware
# app.add_middleware(
#     CORSMiddleware,
...
# async def create_resume(request: Request, background_tasks: BackgroundTasks):
...
#         # Use temp directory for Render (better for ephemeral filesystem)
...
# async def enhance_summary(request: TextRequest):
...
# async def enhance_project(request: TextRequest):
