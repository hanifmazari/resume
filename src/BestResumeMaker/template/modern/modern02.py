...
class ResumePDF:
    def __init__(self, output_file="resume.pdf", page_size="A4", template_image="template.png"):
        ...
        self.content_width = (self.page_width * 2/3) - 50  # 40 = right margin + padding
        
        # Adjust document margins to align with our layout
        self.doc = SimpleDocTemplate(
            output_file, 
            pagesize=self.pagesize,
            rightMargin=20 + self.sidebar_width, 
            leftMargin=20,
            topMargin=20, 
            bottomMargin=20
        )
    ...
    def add_profile_picture(self, base64_image, size=2*inch, border_width=0.1*inch):
        ...
        try:
            ...
            centered_table = Table([[profile_table]], colWidths=[self.sidebar_width - 20])
    ...
    def _add_social_media_to_sidebar(self, personal_info):
        ...
        if 'facebook' in personal_info:
            try:
                ...
                fb_link = personal_info['facebook']
                self.sidebar_content.append(Paragraph(f'<a href="{fb_link}" color="black">Facebook</a>', self.styles['SidebarContactInfo']))
        ...
        if 'linkedin' in personal_info:
            try:
                ...
                li_link = personal_info['linkedin']
                self.sidebar_content.append(Paragraph(f'<a href="{li_link}" color="black">LinkedIn</a>', self.styles['SidebarContactInfo']))
    ...
    def _render_sidebar(self, canvas, is_first_page):
        ...
        sidebar_x = (self.page_width * 2/3) + 10
        ...
        while self.sidebar_content_pos < len(self.sidebar_content) and not force_next_page:
            ...
            if isinstance(element, Spacer):
                ...
                w, h = element.wrap(sidebar_width, frame_height)  # Use full frame height for measurement
            
            # If element fits or it's the first element, add it
            if h <= remaining_height or not elements_to_render:
