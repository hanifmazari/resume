...
class ResumePDF:
    ...
    def _setup_styles(self):
        ...
        styles.add(ParagraphStyle(
            ...
            spaceAfter=6,
            fontName='Helvetica-Bold',
            textColor=colors.HexColor('#ebfbff')
        ))
        ...
        styles.add(ParagraphStyle(
            ...
            leading=20,
            alignment=TA_LEFT,
            borderColor= colors.HexColor("#1A2299"),
            borderWidth=1,
            spaceBefore=12,
            spaceAfter=8,
            fontName='Helvetica-Bold',
    ...
    def _add_contact_to_sidebar(self, personal_info):
        ...
        line_width = self.sidebar_width - 18  # Reduced from 20 to 18 for moving 2 points right
        line = HRFlowable(
            width=line_width,
            thickness=1,
        ...
        self.sidebar_content.append(Indenter(left=-18))  # Reduced from -20 to -18 for moving 2 points right
        ...
        try:
            ...
            ], colWidths=[icon_size + 5, self.sidebar_width - icon_size - 18])  # Reduced from 20 to 18 for moving 2 points right
    ...
    def _add_social_media_to_sidebar(self, personal_info):
        ...
        line_width = self.sidebar_width - 22  # Increased from 20 to 22 for moving 2 points right
        line = HRFlowable(
            width=line_width,
            thickness=1,
            color=colors.HexColor("#D3D3D3"),  # Light gray color
        ...
        if 'facebook' in personal_info:
            try:
                ...
                fb_table = Table([
                    [fb_icon, fb_para]
                ], colWidths=[icon_size + 5, self.sidebar_width - icon_size - 22])  # Increased from 20 to 22 for moving 2 points right
        ...
        if 'website' in personal_info:
            try:
                ...
                web_link = personal_info['website']
                self.sidebar_content.append(Paragraph(f'<a href="{web_link}" color="white">Portfolio</a>', self.styles['SidebarContactInfo']))
        
        self.sidebar_content.append(Indenter(left=18))  # Keep as 18 to balance the left indenter
